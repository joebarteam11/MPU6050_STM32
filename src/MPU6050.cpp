...
void MPU6050::makeAData(const uint8_t* buffer, float* acc) {
    ...
    for (size_t i = 0; i < 3; i++) {
        ...
        acc[i] = ((float)(counts) * aRes - accelBias[i])*9.81f;
    }
}
...
void MPU6050::reset() {
    ...
    writeByte(SIGNAL_PATH_RESET,0x07); //reset all sensors
    thread_sleep_for(100);
}
...
void MPU6050::selfTest(
    ...
    printf("Self Test: \n");
       ...
       printf("%i\n",selfTest[4]); printf("%i\n",selfTest[5]);
    printf("Factory Trim: \n");
       printf("%f\n",factoryTrim[0]); printf("%f\n",factoryTrim[1]);
       printf("%f\n",factoryTrim[2]); printf("%f\n",factoryTrim[3]);
       printf("%f\n",factoryTrim[4]); printf("%f\n",factoryTrim[5]);
